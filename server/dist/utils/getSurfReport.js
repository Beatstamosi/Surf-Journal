 ...
 */
function calculateEnhancedRating(waveMatch, windMatch, tideMatch, spotName) {
    ...
    if (primarySwell?.period) {
        if (primarySwell.period >= 16)
            rating += 1.0; // Excellent groundswell
        else if (primarySwell.period >= 13)
            rating += 0.5; // Good groundswell
        else if (primarySwell.period >= 10)
            rating += 0.25; // Decent swell
        else if (primarySwell.period < 8)
            rating -= 0.5; // Poor windswell
    }
    // Adjust based on swell consistency (multiple swells = worse)
    const activeSwells = waveMatch.swells?.filter((s) => s.height > 0.1).length || 0;
    if (activeSwells >= 2)
        rating -= 0.25;
    if (activeSwells >= 3)
        rating -= 0.25;
    ...
    if (windMatch) {
        ...
        if (windDirection?.includes("offshore"))
            rating += 0.75;
        else if (windDirection?.includes("cross offshore"))
            rating += 0.5;
        else if (windDirection?.includes("onshore"))
            rating -= 0.5;
        else if (windDirection?.includes("cross onshore"))
            rating -= 0.25;
        // Wind speed adjustments
        if (windSpeed < 5)
            rating += 0.25; // Light wind bonus
        else if (windSpeed > 20)
            rating -= 0.5; // Strong wind penalty
        else if (windSpeed > 30)
            rating -= 1.0; // Very strong wind penalty
    }
    // Adjust based on tide
    if (tideMatch?.type) {
        const tideType = tideMatch.type.toLowerCase();
        // Some general adjustments
        if (tideType.includes("low") && avgWaveHeight > 2)
            rating -= 0.25;
        if (tideType.includes("high") && avgWaveHeight < 1)
            rating += 0.25;
    }
 ...
 */
function calculateBaseRatingFromWaveHeight(waveHeight) {
    if (waveHeight < 0.3)
        return 1; // VERY POOR - Flat
    if (waveHeight < 0.8)
        return 1.5; // VERY POOR - Tiny
    if (waveHeight < 1.2)
        return 2; // POOR - Ankle to knee high
    if (waveHeight < 1.8)
        return 2.5; // POOR to FAIR - Knee high
    if (waveHeight < 2.5)
        return 3; // FAIR - Waist high
    if (waveHeight < 3.2)
        return 3.5; // FAIR to GOOD - Chest high
    if (waveHeight < 4.0)
        return 4; // GOOD - Head high
    if (waveHeight < 5.0)
        return 4.5; // GOOD - Overhead
    return 5; // GOOD - Double overhead+
}
/**
 * Spot-specific rating adjustments
 */
function getSpotSpecificAdjustment(spotName, waveHeight) {
